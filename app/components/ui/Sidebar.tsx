...
const Sidebar = () => {
  ...
  if (status === "unauthenticated") {
    return (
      <div className="relative shrink-0">
        <aside
          className="bg-surface border-border sticky top-0 z-3 flex h-screen
  ...
  return (
    <div className="relative shrink-0 md:sticky md:top-0 md:h-dvh">
      {isOpen && (
        <div
          className="fixed inset-0 z-2 bg-black/40 md:hidden"
          onClick={toggleOpen}
          aria-hidden="true"
        />
      )}
      ...
      <button
        ...
        className={`border-border bg-surface hover:bg-accent-hover fixed top-4
          z-4 flex h-9 w-9 cursor-pointer items-center justify-center rounded-xl
          border shadow-sm transition-[left] duration-200 md:absolute md:z-10 ${
      ...
      <aside
        className={`bg-surface border-border fixed top-0 left-0 z-3 flex h-dvh
          flex-col justify-between overflow-hidden border-r shadow-sm
          transition-all duration-200 md:static md:h-full ${
            isOpen
              ? "w-64 translate-x-0"
